...
export default function Vault() {
  ...
  async function loadDocs() {
    const { data } = await supabase
      ...
      .select('id, client_id, name, file_url, file_type, file_size, is_shared, created_at, clients(full_name)')
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const ts = Date.now()
    const path = `vault/${profile.id}/${ts}-${file.name}`
    ...
    <div className="max-w-5xl">
      ...
      <div className="relative h-48 rounded-2xl overflow-hidden mb-8">
        ...
        <div className="relative h-full flex flex-col justify-end px-8 pb-6">
          ...
          <p className="font-barlow text-sm text-white/50 mt-1">Upload training documents to convert into programs</p>
        </div>
      ...
      {parseError && (
        <div className="mb-4 bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-3 flex items-center justify-between">
          <p className="font-barlow text-sm text-red-400">{parseError}</p>
          <button onClick={() => setParseError(null)} className="text-red-400/50 hover:text-red-400 ml-3">×</button>
        </div>
      )}
      ...
      {docs.length === 0 ? (
        <div className="bg-[#1C1C1E] rounded-xl border border-[#2C2C2E] p-16 text-center">
          ...
          <p className="font-barlow text-sm text-white/30">Upload a training program document to get started.</p>
        </div>
      ) : (
                ...
                <div className="flex items-center gap-4">
                  ...
                  {isProgramFile && (
                    <button
                      ...
                      disabled={isParsing}
                      className="bg-[#C9A84C] text-black font-bebas text-xs tracking-widest px-4 py-2 rounded-lg hover:bg-[#E2C070] transition-colors disabled:opacity-50 flex-shrink-0"
                    >
                      {isParsing ? 'Parsing...' : 'Convert to Program'}
                    </button>
                  )}
                  ...
                  <button
                    ...
                    className="font-barlow text-xs text-red-400/40 hover:text-red-400 transition-colors flex-shrink-0"
                  >
                    Delete
                  </button>
                </div>
