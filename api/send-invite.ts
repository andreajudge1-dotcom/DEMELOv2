...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  console.log('send-invite called, method:', req.method)
  ...
  if (!to_email || !to_name || !trainer_name || !register_url) {
    console.log('Missing fields', { to_email, to_name, trainer_name, register_url })
    return res.status(400).json({ error: 'Missing required fields' })
  }

  const apiKey = process.env.RESEND_KEY
  console.log('API key present:', !!apiKey)
  console.log('Env var keys:', Object.keys(process.env).join(', '))
  ...
  try {
    ...
    if (!response.ok) {
      return res.status(500).json({ error: data?.message ?? data?.name ?? 'Failed to send email' })
    }
